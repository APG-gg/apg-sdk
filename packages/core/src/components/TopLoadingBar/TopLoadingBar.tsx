...
export type TopLoadingBarProps = {
  ...
  externalRef?: React.Ref<TopLoadingBarRef>
}
...
const TopLoadingBar: FC<TopLoadingBarProps>  = forwardRef<TopLoadingBarRef, TopLoadingBarProps>(
  (
    {
      progress,
      height = 2,
    ...
    useEffect(() => {
      if (externalRef) {
        if (externalRef && progress !== undefined) {
          console.warn(
